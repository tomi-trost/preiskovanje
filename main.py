...
class Graph:
    ...
    def __init__(self, start_state: np.ndarray, finish_state: np.ndarray, dimensions: dict[str:int]):
        ...
        self.combinations = math.factorial(
            dimensions['n']+dimensions['p']-1) / math.factorial(dimensions['p']-1)
    ...
    class Node:
        ...
        def __init__(self, graph: 'Graph', state: np.ndarray, past_state: dict[tuple:any]):
            ...
            self.possible_future_paths: list[tuple] = [(p, r) for r in range(
                self.graph.dimensions['p']) for p in range(self.graph.dimensions['p'])]
        ...
        def move(self, p: int, r: int) -> np.ndarray:
            ...
            if not self.valid_move(p, r):
                raise Exception(
                    "Invalid move. Index p and/or r out of bounds of matrix.")
            ...
            try:
                ...
                sub_index = substitute_column.index(
                    next(x for x in substitute_column if x != ' '))
                ...
                place_index = len(place_column) - \
                    place_column[::-1].index(' ') - 1
        ...
        def future_step(self):
            ...
            legal_future_paths: list[tuple] = [
                ...
                all([not np.array_equal(self.move(p, r), visited)
                    for visited in self.graph.visited])
            ]
            ...
            self.future = {(p, r): self.graph.Node(self.graph, self.move(
                p, r), {(p, r): self}) for p, r in legal_future_paths}

            # dodajam nek komentar
...
def read_example(file_name: str) -> np.ndarray:
    with open(file_name) as file:
        ...
        lines = [[element[1:-1] for element in line.strip().split(',')]
...
for example in examples:
    ...
    # print("start:\n"+ str(example[0]) + ",\n\n end:\n"+ str(example[1]))
    test = Graph(start_state=example[0], finish_state=example[1],
                 dimensions=get_dimensions(example[0]))
